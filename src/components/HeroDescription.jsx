import AnimatedTextCharacter from "@/components/AnimatedText";
import Button from "./Buttons";


export default function HeroDescription({ title, animatedHeading, desc, modalAction }) {

    return (
        <>
            <span data-aos="fade-up" className="heading-1 h2">{title && title}<span className="block">
                <AnimatedTextCharacter text={animatedHeading && animatedHeading} />
            </span>
            </span>
            <p data-aos="fade-up" data-delay="500" className="sm:max-w-[70%] mx-auto" dangerouslySetInnerHTML={{ __html: desc && desc }} />
            <div className='mt-3 flex items-center'>
               {modalAction && <Button
                size="normal"
                label="Let&apos;s start"
                icon={true}
                action={modalAction}
                classes="mx-auto"
                />}
            </div>
        </>
    )

}